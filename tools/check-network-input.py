#!/usr/bin/env python3
"""Type into the actual layer-shell network popover on an isolated mock bus."""
import json
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="wm-network-input-") as temp:
        directory = Path(temp)
        ...
        with log_path.open("w") as log:
            ...
            try:
                ...
                command = ["env", "GDK_BACKEND=wayland", "WM_NETWORK_TEST_PRIVATE_BUS=1", "WM_NETWORK_TEST_INPUT=1",
                           ...
                           "wifi::tests::live_discovery_activation_and_changes", "--ignored", "--test-threads=1"]
                ...
                while not receipt.exists():
                    ...
                    time.sleep(.1)
                print("PASS: real layer-shell popup password typing, profile requests and keyboard-mode restoration")
            finally:
