...
impl IsAlive for KeyboardFocusTarget {
    ...
    fn alive(&self) -> bool {
        match self {
            KeyboardFocusTarget::Lock(w) => w.alive(),
...
impl PointerFocusTarget {
    fn allowed_while_locked<B: Backend>(&self, data: &AnvilState<B>) -> bool {
        if !data.lock.locked {
            return true;
        }
        let Self::WlSurface(surface) = self else {
            return false;
        };
        data.space.outputs().any(|output| {
            output
                .user_data()
                .get::<std::sync::Mutex<crate::lock::LockOutput>>()
                .is_some_and(|lock| lock.lock().unwrap().surface.as_ref() == Some(surface))
        })
    }
...
impl<BackendData: Backend> KeyboardTarget<AnvilState<BackendData>> for KeyboardFocusTarget {
    fn enter(
        ...
        if data.lock.locked && !matches!(self, Self::Lock(_)) {
            return;
        }
...
impl<BackendData: Backend> TabletToolTarget<AnvilState<BackendData>> for PointerFocusTarget {
    fn proximity_in(
        ...
        if !self.allowed_while_locked(data) {
            return;
        }
...
impl WaylandFocus for KeyboardFocusTarget {
    ...
    fn wl_surface(&self) -> Option<Cow<'_, WlSurface>> {
        match self {
            KeyboardFocusTarget::Lock(w) => Some(Cow::Borrowed(w)),
...
impl From<KeyboardFocusTarget> for PointerFocusTarget {
    ...
    fn from(value: KeyboardFocusTarget) -> Self {
        match value {
            ...
            KeyboardFocusTarget::Lock(w) => PointerFocusTarget::WlSurface(w),
