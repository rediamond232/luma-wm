...
impl<BackendData: Backend> AnvilState<BackendData> {
    ...
    fn process_common_key_action(&mut self, action: KeyAction) {
        match action {
            KeyAction::None => (),
            KeyAction::Desktop(command) => {
                if let Err(e) = self.desktop_command(&command) {
        ...
        if self.lock.locked {
            let action = keyboard
                .input(
                    ...
                    time,
                    |_, modifiers, handle| {
                        let symbol = handle.modified_sym();
                        if state == KeyState::Pressed {
                            if let Some(action) = process_keyboard_shortcut(*modifiers, symbol) {
                                suppressed_keys.push(symbol);
                                return FilterResult::Intercept(action);
                            }
                        } else if suppressed_keys.contains(&symbol) {
                            suppressed_keys.retain(|key| *key != symbol);
    ...
    pub fn surface_under(
        ...
        if self.lock.locked {
            return output
                ...
                .and_then(|s| s.lock().unwrap().surface.clone())
                .map(|s| (PointerFocusTarget::WlSurface(s), output_geo.loc.to_f64()));
        }
        let layers = layer_map_for_output(output);
...
impl<BackendData: Backend> AnvilState<BackendData> {
    ...
    fn on_pointer_move_absolute_windowed<B: InputBackend>(
        ...
        let transform = output.current_transform();
        let native_size = transform.invert().transform_size(output_geo.size);
        let pos = transform
...
fn process_keyboard_shortcut(_modifiers: ModifiersState, keysym: Keysym) -> Option<KeyAction> {
