...
impl<BackendData: Backend> XwmHandler for AnvilState<BackendData> {
    ...
    fn fullscreen_request(&mut self, _xwm: XwmId, window: X11Surface) {
        if let Some(w) = self
            ...
            .windows
            .iter_mut()
            .find(|w| w.window.0.x11_surface() == Some(&window))
        {
            w.fullscreen = true;
        }
    ...
    fn resize_request(
        ...
        if !self.can_manipulate_window(element) {
            return;
        }
...
impl<BackendData: Backend> AnvilState<BackendData> {
    ...
    pub fn move_request_x11(&mut self, window: &X11Surface) {
        if !self.desktop.windows.iter().any(|w| {
            w.window.0.x11_surface() == Some(window) && self.can_manipulate_window(&w.window)
        }) {
            return;
        }
