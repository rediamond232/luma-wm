...
impl<BackendData: Backend> XdgShellHandler for AnvilState<BackendData> {
    ...
    fn resize_request(
        ...
        if self
            ...
            .is_none_or(|window| !self.can_manipulate_window(&window))
        {
            return;
        }
        let seat: Seat<AnvilState<BackendData>> = Seat::from_resource(&seat).unwrap();
    ...
    fn fullscreen_request(
        ...
        if let Some(w) = self
            ...
            .windows
            .iter_mut()
            .find(|w| w.window.0.toplevel() == Some(&surface))
        {
            w.fullscreen = true;
        }
...
impl<BackendData: Backend> AnvilState<BackendData> {
    ...
    pub(super) fn unconstrain_popup(&self, popup: &PopupSurface) {
        ...
        let Some(window) = self.window_for_surface(&root) else {
            // Layer-shell parents are not Space windows. Their geometry is
            // output-local, while the positioner's bounds are parent-local.
            for output in self.space.outputs() {
                ...
                let mut target = smithay::utils::Rectangle::from_size(output_geo.size);
