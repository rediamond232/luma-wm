...
#[derive(Default)]
pub struct WindowCornerRadius(pub std::sync::Mutex<f64>);
#[derive(Default)]
pub struct WindowBlur(pub std::sync::Mutex<bool>);
pub struct WindowOpacity(pub std::sync::Mutex<f32>);
pub struct WindowOpening(pub std::sync::Mutex<f32>);
impl Default for WindowOpening {
    fn default() -> Self {
        Self(std::sync::Mutex::new(1.0))
    }
}
...
impl WindowElement {
    fn in_clipped_corner(&self, location: Point<f64, Logical>) -> bool {
        let Some(radius) = self.0.user_data().get::<WindowCornerRadius>() else {
            return false;
        };
        let radius = *radius.0.lock().unwrap();
        if radius <= 0. {
            return false;
        }
        let rect = self.geometry();
        rect.to_f64().contains(location)
            && !wm_core::rounded_contains(
                wm_core::Rect {
                    x: rect.loc.x,
                    y: rect.loc.y,
                    w: rect.size.w,
    ...
    pub fn surface_under(
        ...
        if self.in_clipped_corner(location) {
            return None;
        }
...
impl SpaceElement for WindowElement {
    ...
    fn is_in_input_region(&self, point: &Point<f64, Logical>) -> bool {
        if self.in_clipped_corner(*point) {
            return false;
        }
        if self.decoration_state().is_ssd {
    ...
    fn render_elements<C: From<Self::RenderElement>>(
        ...
        let alpha = alpha
            * self
                ...
                .user_data()
                .get::<WindowOpacity>()
                .map(|opacity| *opacity.0.lock().unwrap())
                .unwrap_or(1.0);
