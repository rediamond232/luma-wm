...
fn remember_move<B: Backend>(
    data: &mut AnvilState<B>,
    window: &WindowElement,
    location: Point<i32, Logical>,
) {
    if let Some(managed) = data
        .desktop
        .windows
        .iter_mut()
        .find(|w| &w.window == window)
    {
        if managed.floating && !managed.fullscreen {
            if let Some(mut rect) = managed.floating_rect.or(managed.rect) {
                rect.x = location.x;
                rect.y = location.y;
...
fn remember_resize<B: Backend>(
    ...
    {
        if managed.floating && !managed.fullscreen {
            managed.floating_rect = Some(wm_core::Rect {
                x: initial_location.x
                    + if edges.contains(ResizeEdge::LEFT) {
                        initial_size.w - size.w
                    } else {
                        0
                    },
                y: initial_location.y
                    + if edges.contains(ResizeEdge::TOP) {
                        initial_size.h - size.h
                    } else {
                        0
                    },
                w: size.w,
...
impl<B: Backend + 'static> AnvilState<B> {
    pub fn begin_mouse_action(&mut self, button: u32, serial: Serial) {
        use smithay::input::pointer::Focus;
        if self.lock.locked || self.pointer.is_grabbed() || ![0x110, 0x111].contains(&button) {
            return;
        }
        ...
        let enabled = match self.desktop.config.input.mouse_modifier.as_str() {
        ...
        let window = match focus.as_ref().map(|(target, _)| target) {
            Some(PointerFocusTarget::WlSurface(surface)) => self.window_for_surface(surface),
            #[cfg(feature = "xwayland")]
            Some(PointerFocusTarget::X11Surface(surface)) => self
                .desktop
                .windows
                .iter()
                .find(|w| w.window.0.x11_surface() == Some(surface))
                .map(|w| w.window.clone()),
            _ => None,
        };
        let Some(window) = window.filter(|w| self.can_manipulate_window(w)) else {
            return;
        };
        ...
        if button == 0x110 {
            ...
            let edges =
                if location.x < (initial_window_location.x + initial_window_size.w / 2) as f64 {
                    ResizeEdge::LEFT
                } else {
                    ResizeEdge::RIGHT
                } | if location.y < (initial_window_location.y + initial_window_size.h / 2) as f64 {
                    ResizeEdge::TOP
                } else {
                    ResizeEdge::BOTTOM
                };
    ...
    for PointerMoveSurfaceGrab<BackendData>
{
    fn motion(
        ...
        remember_move(data, &self.window, new_location.to_i32_round());
        data.space
