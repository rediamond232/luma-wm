...
#[cfg(feature = "debug")]
use crate::drawing::{FPS_NUMBERS_PNG, FpsElement};
use crate::{
    render::*,
...
pub fn run_x11() {
    ...
    let window = WindowBuilder::new()
        .title(
            &std::env::var("WM_NESTED_TITLE").unwrap_or_else(|_| "wm — development session".into()),
        )
        .build(&handle)
    ...
    event_loop
        ...
        .insert_source(backend, move |event, _, data| match event {
            ...
            X11Event::PresentCompleted { .. } => {
                // Presentation frees the buffer; it is not new scene damage.
                data.backend_data.render = true;
            }
            X11Event::Refresh { .. } => {
                data.backend_data.render = true;
            ...
            X11Event::Input { event, .. } => {
                data.process_input_event_windowed(event, OUTPUT_NAME);
                data.desktop.redraw = true;
            }
    ...
    while state.running.load(Ordering::SeqCst) {
        ...
        if state.backend_data.render && state.desktop.redraw {
            ...
            elements.extend(pointer_element.render_elements(&mut backend_data.renderer, &output));
